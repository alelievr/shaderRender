...
SPointLight lightPoint1 =
	SPointLight
	(
		vec3(sin(1.57 + iGlobalTime*1.5),sin(iGlobalTime*0.5),cos(1.57 + iGlobalTime*1.5)),
		vec3(0.9,0.9,0.9)
	);
...
SAxisAlignedBox orbitBox = 
	...
	(
		3,						//Id
		vec3(sin(2.0 + iGlobalTime*1.3),0.0,cos(2.0 + iGlobalTime*1.3)),	//center
...
SAxisAlignedBox orbitBox2 = 
	...
	(
		4,						//Id
		vec3(sin(5.14 + iGlobalTime*1.3),0.0,cos(5.14 + iGlobalTime*1.3)),	//center
		vec3(0.5,0.75,0.5),	//scale
		SMaterial
		(
			0.25,				//diffuse amount
...
bool RayIntersectSphere (inout SSphere sphere, inout SCollisionInfo info, in vec3 rayPos, in vec3 rayDir, in int ignorePrimitiveId)
{
	...
	float A = texture2D(iChannel0, info.m_textureCoordinates + vec2(0.0, 0.0)).x;
	float B = texture2D(iChannel0, info.m_textureCoordinates + vec2(delta, 0.0)).x;
    float C = texture2D(iChannel0, info.m_textureCoordinates + vec2(0.0, delta)).x;
...
bool RayIntersectAABox (inout SAxisAlignedBox box, inout SCollisionInfo info, in vec3 rayPos, in vec3 rayDir, in int ignorePrimitiveId)
{
	...
	float delta = -1.0/512.0;
	if (box.m_Id == 4) {
		float A = texture2D(iChannel1, info.m_textureCoordinates + vec2(0.0, 0.0)).x;
		float B = texture2D(iChannel1, info.m_textureCoordinates + vec2(delta, 0.0)).x;
    	float C = texture2D(iChannel1, info.m_textureCoordinates + vec2(0.0, delta)).x;
		vec3 norm = normalize(vec3(B - A, C - A, 0.25));
		info.m_surfaceNormal = normalize(mattanspace * norm);
	}
	else {
	...
	for(int index = 0; index < c_maxRayBounces; ++index)
	{
		...
		{			
			if (collisionInfo.m_Id == 4)
				collisionInfo.m_material.m_diffuseColor = texture2D(iChannel1, collisionInfo.m_textureCoordinates).xyz;
			else
				collisionInfo.m_material.m_diffuseColor = texture2D(iChannel0, collisionInfo.m_textureCoordinates).xyz;
		...
		{
			pixelColor+= colorMultiplier * texture2D(iChannel2, rayDir.xy).rgb;
			return;
		}
	}
