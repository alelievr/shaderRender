#pragma iChannel0 textures/grey-cells.png
#pragma iChannel1 textures/Kifs1.jpg
/*
...
float map(vec3 p){
    ...
    return (n - p.x*p.y*p.z*.35 - .9)*1.4; // Combine, and we're done.
...
void mainImage( in vec2 fragCoord ){
	...
	vec3    r = normalize(uv.x * right + uv.y * up + fov * forw);
	...
	// Camera Setup.
    float speed = 4.;
    vec3 l = o;// + vec3(sin(iGlobalTime / 1), cos(iGlobalTime / 1), 0);
    ...
    if(t<FAR){
        ...
        // refraction vectors at the surfact, then pass them to the environmapping function.
        // Note that glass and fluid have different refractive indices, so I've fudged them
        // into one figure.
        ...
        */
        ...
        // diffuse value.
        col += refCol*((di*di*.5+.5) + ao*.25)*1.5; // Add the reflected color. You could combine it in other ways too.
    ...
    //vec3 fog = vec3(.5, .1, .05)*(r.y*.5 + .5);    
    //col = mix(col, fog/4., smoothstep(0., .95, t/FAR)); // exp(-.002*t*t), etc. fog.zxy
    
    // Subtle vignette.
    u = fragCoord/iResolution.xy;
    col = mix(vec3(0), col, pow( 16.0*u.x*u.y*(1.0-u.x)*(1.0-u.y) , .125));
