...
#define time ((sin(float(__LINE__))/PI/GR+1.0/GR/PI/E)*iGlobalTime+1000.0)
...
vec4 galaxy(vec2 uv)
{
    ...
    float rotation = iGlobalTime*PI*2.0;
...
vec4 mainImage2( in vec2 fragCoord )
{
    ...
    vec4 back = texture2D(iChannel0, fragCoord/iResolution.xy+d);
    float skew = (120.0+64.0)/255.0;
    if(color.a <= skew)
        return clamp((back*(1.0-color.a)/skew+color*color.a/skew), 0.0, 1.0);
    else
    	return vec4(color)*GR+(1.0-color.a)*back;
}

void mainImage( in vec2 fragCoord )
{
	fragColor = clamp(mainImage2(fragCoord) -1.0/60.0, 0.0, 1.0);
}
